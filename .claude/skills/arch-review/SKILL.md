...
| Axis | Where | Rule |
|---|---|---|
| **UI composition** — Atomic Design | `shared/ui/{atoms,molecules,organisms}` | Presentation only. Knows nothing about HSK. Never fetches. |
...
- **A screen belongs to its feature, not to `app/`.** Every route file is the same five lines —
  resolve `params`, render the screen inside `LessonGate`. A route that grows a helper function, a
...
```bash
npm run test:pinyin    # syllable splitting + tone sandhi — the one place silent wrongness hides
...
1. **Does it read domain state or call domain code?** `useAuth`, `useProgress`, `useLesson`, Firestore,
   `localStorage` → it belongs to a **feature**, never to `shared/ui`. This is the mistake the
   Atomic Design note calls out by name.
2. **Does it know the shape of our data?** Touching `word` / `pinyin` / `meaningVi` / `titleZh` /
   `collections` / `volume` / `slug` counts as domain knowledge → **feature**.
   *Precedent: `AccountBar` looks like an organism and is not — it lives in `features/auth` for
   exactly this reason.*
   *Borderline worth naming: tone helpers in `shared/lib/pinyin.js` are pure string functions and
   stay shared; anything that reads `item.pinyin` off a content item is domain and moves out.*
...
4. **Is the file doing two jobs?** `schema.js` (shape + assembly), `bundled.js` (generated data)
   and `activities.js` (which sections a level has) are deliberately three files, not one. A new
   file mixing data access with a hook, or a hook with a presentation component, should be split
   along the same line.

5. **Does it hardcode the section list?** The seven 板块 differ between level 1–3 and level 4+ —
   same count, different set. Any component enumerating sections instead of calling
   `sectionsForLevel(level)` is a blocking finding: it will render level 4 wrong.
...
- **`merge.js` imports nothing.** It is meant to run under bare `node` in a test, same as
  `shared/lib/pinyin.js` does in `scripts/test-pinyin.mjs`. An import of `@/…` there breaks that.
- **Generated files are not edited by hand.** `features/lesson/bundled.js` carries a header saying
  so. A diff that touches it without a matching change under `content/` means someone edited the
  output instead of re-running `npm run gen`.
...
Content rules (`source`, `schemaVersion`, id stability, answer keys) belong to **`hsk-import`**,
